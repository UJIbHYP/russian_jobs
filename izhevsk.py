...
def engine(login, psw, str_zapros, page):
	...
	browser.find_element(By.XPATH, '//*[@id="loginByPwdButton"]').click()
	time.sleep(2)
	...
	params={
		"filter":'{"title":["'+str_zapros+'"],"regionCode":[""],"districts":["9400000000000"],"publishDateTime":["EXP_MAX"],"titleType":["CV_ALL"]}',
...
[sg.Text("ГОСУСЛУГИ: ")],
[sg.Text("Логин ",text_color="#cccccc"), sg.Input("",size=(15,1),  key="login_gsg") ],
[sg.Text("Пароль ",text_color="#cccccc"), sg.Input("", size=(15,1), key="psw_gsg") ],
[sg.Text("")],
[sg.Text("Запрос ",text_color="#dddddd"), sg.Input("",size=(22,1),  key="zapros")],

[sg.Text("")],
[sg.Button("Запустить выгрузку.")],
[sg.Text("")],
[sg.Text(str(datetime.datetime.now()), key="status")  ],
[sg.Text("")],
...
while True:
	...
	if event == "Запустить выгрузку." :
		...
		if int(time.time())<1669065390:
			engine(login_gsg, psw_gsg, zapros,0)
			window.Element("status").update(value="В процессе.")

			pass
		else:
			window.Element("status").update(value="Нужно обновление!")
		pass
